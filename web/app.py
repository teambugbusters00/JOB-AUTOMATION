...
from job_agent.collectors.himalayas import collect as collect_himalayas
from job_agent.collectors.greenhouse import collect as collect_greenhouse
from job_agent.collectors.lever import collect as collect_lever
from job_agent.collectors.ashby import collect as collect_ashby
...
app = FastAPI(title="JOB-AUTOMATION API", version="2.0.0")
...
def current_user(session: str|None):
    ...
    user=require_db().session_user(token_hash)
...
def register(payload:dict[str,Any],response:Response):
    ...
    repo=require_db()
    try: user=repo.create_user(email,password_hash)
...
def login(payload:dict[str,Any],response:Response):
    ...
    repo.set_login(user["id"]); token,token_hash,expires=new_session(); repo.create_session(user["id"],token_hash,expires)
    response.set_cookie(SESSION_COOKIE,token,max_age=14*24*3600,httponly=True,samesite="lax",secure=os.getenv("ENVIRONMENT","production") == "production",path="/")
...
def get_profile(job_session: str|None=Cookie(default=None)):
    user=current_user(job_session); return require_db().get_profile(user["id"])

@app.put("/api/profile")
def put_profile(payload:dict[str,Any],job_session: str|None=Cookie(default=None)):
    user=current_user(job_session); profile=dict(payload); profile["onboarding_complete"]=True; require_db().save_profile(user["id"],profile); return require_db().get_profile(user["id"])
...
def rag_status(job_session: str|None=Cookie(default=None)):
    ...
    with require_db().conn() as c:
        ...
        count=c.execute("SELECT COALESCE(SUM(chunks),0) AS chunks,COUNT(*) AS documents FROM rag_documents").fetchone(); c.commit()
...
@app.get("/api/portals")
def portals():
    repo=require_db(); counts={x["source"]:x for x in repo.portal_counts()}; cards=[]
    for p in portal_cards(): cards.append({**p,"job_count":int(counts.get(p["name"],{}).get("jobs",0) or 0),"last_seen":counts.get(p["name"],{}).get("last_seen")})
    return cards

@app.get("/api/portals/runs")
def portal_runs(job_session: str|None=Cookie(default=None)):
    current_user(job_session); return [dict(x) for x in require_db().portal_runs(100)] if hasattr(require_db(),"portal_runs") else []
...
def screen_portal(portal_key:str,job_session: str|None=Cookie(default=None)):
    ...
    try:
        profile=repo.get_profile(user["id"]); raw=collector(); raw=[normalize(j) for j in raw]
        if profile.get("india_eligibility_required",True): raw=[j for j in raw if india_eligible(j) or portal_key in {"thehub","workinstartups","startupmap"}]
        ...
        for s,j,reasons in ranked:
            repo.upsert(j,s,reasons); saved+=1
            if s>=85: repo.queue_application(repo.upsert(j,s,reasons),"software-engineering",user["id"])
