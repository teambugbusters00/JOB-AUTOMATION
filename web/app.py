...
app=FastAPI(title="JOB-AUTOMATION API",version="2.0.2")
...
def register(payload:dict[str,Any],response:Response):
    ...
    token,token_hash,expires=new_session();repo.create_session(user["id"],token_hash,expires);response.set_cookie(SESSION_COOKIE,token,max_age=14*24*3600,httponly=True,samesite="lax",secure=os.getenv("ENVIRONMENT","production")=="production",path="/")
...
def login(payload:dict[str,Any],response:Response):
    ...
    repo.set_login(user["id"]);token,token_hash,expires=new_session();repo.create_session(user["id"],token_hash,expires);response.set_cookie(SESSION_COOKIE,token,max_age=14*24*3600,httponly=True,samesite="lax",secure=os.getenv("ENVIRONMENT","production")=="production",path="/");user=repo.get_user(user["id"])
    return {"user":public_user(user),"profile":repo.get_profile(user["id"])}
...
@app.get("/api/profile")
def get_profile(job_session:str|None=Cookie(default=None)): return require_db().get_profile(current_user(job_session)["id"])
@app.put("/api/profile")
...
@app.get("/api/portals")
def portals():
    counts={str(x["source"]).lower():x for x in require_db().portal_counts()};aliases={"himalayas":["himalayas"],"greenhouse":["greenhouse"],"lever":["lever"],"ashby":["ashby"],"startup_jobs":["startup jobs"],"topstartups":["top startups"],"thehub":["the hub"],"workinstartups":["work in startups"],"startupmap":["startupmap"]}
    cards=[]
    for p in portal_cards():
        keys=aliases.get(p.key,[p.name.lower()]);hits=[counts[k] for k in keys if k in counts];cards.append({**p.__dict__,"job_count":sum(int(x["jobs"] or 0) for x in hits),"last_seen":max((x["last_seen"] for x in hits if x["last_seen"]),default=None)})
    return cards
@app.get("/api/portals/runs")
def portal_runs(job_session:str|None=Cookie(default=None)): return [dict(x) for x in require_db().portal_runs(100)] if current_user(job_session) else []
...
def screen_portal(portal_key:str,job_session:str|None=Cookie(default=None)):
    ...
    try:
        profile=repo.get_profile(user["id"]);raw=[normalize(j) for j in collector()];ranked=sorted(((score_with_profile(j,profile),j,explain_with_profile(j,profile)) for j in dedupe(raw)),key=lambda x:x[0],reverse=True);saved=0
        ...
        repo.log_portal_run(portal.name,"completed",len(ranked),saved);return {"status":"completed","portal":portal.name,"discovered":len(ranked),"saved":saved,"jobs":[{"title":j.title,"company":j.company,"score":s,"url":j.url} for s,j,_ in ranked[:20]]}
