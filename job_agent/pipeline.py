...
def profile_eligible(job,profile):
    ...
    text=f"{job.title} {job.employment_type} {job.description} {job.location} {job.remote}".lower()
    if profile.get("india_eligibility_required",True) and not india_eligible(job):
        remote_pref=[str(x).lower() for x in profile.get("remote_modes",[])]; country_pref=[str(x).lower() for x in profile.get("preferred_countries",[])]
        if not any(x in text for x in remote_pref+country_pref+['worldwide','global','remote']): return False
    mode=str(profile.get("experience_mode") or "both").lower()
    if mode=="internship" and not any(x in text for x in ["intern","internship","student"]): return False
    if mode=="full-time" and any(x in text for x in ["internship","intern"]): return False
    return True
...
def _report(ranked,total,eligible,queued=0):
    ...
    for s,job,reasons in ranked[:10]:lines += [f"{s}% | {job.company} | {job.title}",f"     {job.location} | {job.source}",f"     ✓ {', '.join(reasons[:6])}" if reasons else "",f"     {job.url}"]
    return "\n".join(lines)

def run(profile=None,user_id=None):
    raw=collect_all(profile);eligible=[j for j in raw if profile_eligible(j,profile)];jobs=dedupe(eligible)
    scorer=(lambda j:score_with_profile(j,profile)) if profile else score;explainer=(lambda j:explain_with_profile(j,profile)) if profile else explain
    ranked=sorted(((scorer(j),j,explainer(j)) for j in jobs),key=lambda x:x[0],reverse=True);queued=0
    db_url=os.getenv("DATABASE_URL")
    if db_url:
        repo=JobRepository(db_url);repo.init()
        for s,job,reasons in ranked:
            job_id=repo.upsert(job,s,reasons)
            if s>=85:repo.queue_application(job_id,resume_variant=_resume_variant(job),user_id=user_id);queued+=1
